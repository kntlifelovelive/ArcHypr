...
# Hyprland Used Hyprlock Waybar Power Menu
# --- Configuration ---
...
check_dependencies() {
  ...
  if ! command -v hyprlock &>/dev/null; then
    echo "Warning: hyprlock is not installed. Lock function will not work."
    notify-send "Warning: hyprlock is not installed. Lock function will not work."
  fi
}
...
do_lock() {
  zenity --question \
    ...
    --text="Do you want to Lock Screen Hyprland?" \
    --ok-label="Exit" \
    --cancel-label="Cancel"

  if [ $? -eq 0 ]; then
    echo "Locking Hyprland session..."
    $LOCK_CMD
  else
    echo "Session lock cancelled."
  fi
}
...
# Function for exiting session (Hyprland)
do_logout() {
  zenity --question \
    --title="Logout Confirm" \
    --text="Do you want to exit Hyprland session?" \
    --ok-label="Exit" \
    --cancel-label="Cancel"
